...
try:
    ...
    if result_df.empty:
        ...
        # --- マップ作成（全員を表示、1位・最新をハイライト） ---
        st.divider()
        st.subheader("🗺️ 全員の進路予想マップ")
        st.info("1位（赤）、最新の応募者（青）、その他（グレー）の経路を表示しています。")
        
        map_df = result_df
        
        # 1位と最新の応募者の「氏名」を取得
        winner_name = result_df.iloc[0]['氏名']
        latest_name = recent_df.iloc[0]['氏名']

        m = folium.Map(location=[seikai_lat_72h, seikai_lon_72h], zoom_start=5, tiles='CartoDB positron', attribution_control=False)

        # 実際の経路 (これはアニメーション線 AntPath のまま)
        ...
        folium.Marker(location=actual_path[-1], icon=folium.Icon(color='black', icon='flag'), popup='最終到達点').add_to(m)

        # --- ★★★ 1位と最新のピンのみ描画 ★★★ ---
        winner_row = result_df.iloc[0]
        latest_row = recent_df.iloc[0]

        if winner_row['氏名'] == latest_row['氏名']:
            # 1位 かつ 最新 の場合
            folium.Marker(
                location=[winner_row['96時間後の予想緯度（北緯）'], winner_row['96時間後の予想経度（東経）']],
                icon=folium.Icon(color='purple', icon='user'), # 色を紫に
